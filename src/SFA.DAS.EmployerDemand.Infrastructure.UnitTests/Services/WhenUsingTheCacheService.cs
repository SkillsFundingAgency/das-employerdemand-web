...
namespace SFA.DAS.EmployerDemand.Infrastructure.UnitTests.Services
{
    public class WhenUsingTheCacheStorageService
    {
        ...
        {
            //Act
            await service.SaveToCache(keyName, test, TimeSpan.FromHours(expiryInHours));
            
            //Assert
            distributedCache.Verify(x=>
                x.SetAsync(
                    $"SFA.DAS.EmployerDemand_{keyName}",
        ...
        {
            //Arrange
            distributedCache.Setup(x => x.GetAsync($"SFA.DAS.EmployerDemand_{keyName}", It.IsAny<CancellationToken>()))
        ...
        {   
            ...
            //Assert
            distributedCache.Verify(x=>x.RemoveAsync($"SFA.DAS.EmployerDemand_{keyName}", It.IsAny<CancellationToken>()), Times.Once);
        }
